...
# CD34+
CD34.data <- Read10X(data.dir = "~/AML6_Dx_CD34pos/filtered_feature_bc_matrix/")
...
p2 + geom_vline(xintercept=lower_bound) + geom_hline(yintercept=400) + geom_hline(yintercept=6000)

# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD34 <- subset(x = CD34, subset = nFeature_RNA > 400 & nFeature_RNA < 6000  & percent.mito < 0.2 & log10_nCount_RNA > lower_bound)
CD34

# CD38+
CD38.data <- Read10X(data.dir = "~/AML6_Dx_CD38pos/filtered_feature_bc_matrix/")
...
# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD38 <- subset(x = CD38, subset = nFeature_RNA > 400 & nFeature_RNA < 5500 & percent.mito < 0.2 & log10_nCount_RNA > lower_bound )
CD38

#### 2. Combine the datasets #### 
combined.obj.AML <- merge(x = CD34, y = CD38, add.cell.ids = c("CD34","CD38") , project = "AML6_Dx")
...
#Filter the AML6_Dx cells (AML6 on Velten table)
Velten_proj_f <- Velten_proj[which(Velten_proj$patient=="AML6"),]
#Format the bc
