...
## 1. AML4_Dx-AML4_Rx: Take the LSC clusters we derived independently
...
#### 1. Load samples processed from AML4_Dx.Rmd and AML4_Rx.Rmd
AML4_Dx$orig.name <- "AML4_Dx"
AML4_Dx$old_clustering <- paste0("AML4_Dx_",Idents(AML4_Dx))
AML4_Dx$Cond <- "Dx"

AML4_Rx$orig.name <- "AML4_Rx"
AML4_Rx$old_clustering <- paste0("AML4_Rx_",Idents(AML4_Rx))
AML4_Rx$Cond <- "Rx"

#### 2.  Integrate the datasets
dt.list <- unlist(list(c(AML4_Dx,AML4_Rx)))
...
all_markers_LSC_AML4 <- FindMarkers(integrated.obj1, ident.1 = "AML4_Dx_9", ident.2 = "AML4_Rx_12", only.pos = FALSE, logfc.threshold = 0)
all_markers_LSC_AML4$cluster <- unlist(lapply(all_markers_LSC_AML4$avg_log2FC,function(x)ifelse(x>0,"AML4_Dx_9","AML4_Rx_12")))
all_markers_LSC_AML4$avg_log2FC <- abs(all_markers_LSC_AML4$avg_log2FC)
all_markers_LSC_AML4$gene <- rownames(all_markers_LSC_AML4)
table(all_markers_LSC_AML4[which(all_markers_LSC_AML4$avg_log2FC>=0.25),]$cluster)
...
aux_df1$LSC_status[which(aux_df1$old_clustering=="AML4_Dx_9" | aux_df1$old_clustering=="AML4_Rx_12")] <- "LSC 34+"
...
LSC_status[which(integrated.obj1$old_clustering=="AML4_Dx_9" | integrated.obj1$old_clustering=="AML4_Rx_12")] <- "LSC 34+"
...
## 2. AML6_Dx-AML6_Rx: Take the LSC clusters we derived independently
...
#### 1. Load samples processed from AML6_Dx.Rmd and AML6_Rx.Rmd
AML6_Dx$orig.name <- "AML6_Dx"
AML6_Dx$old_clustering <- paste0("AML6_Dx_",Idents(AML6_Dx))
AML6_Dx$Cond <- "Dx"

AML6_Rx$orig.name <- "AML6_Rx"
AML6_Rx$old_clustering <- paste0("AML6_Rx_",Idents(AML6_Rx))
AML6_Rx$Cond <- "Rx"

#### 2.  Integrate the datasets
dt.list <- unlist(list(c(AML6_Dx,AML6_Rx)))
for (i in 1:length(dt.list)) {
    dt.list[[i]] <- NormalizeData(dt.list[[i]], verbose = FALSE)
    dt.list[[i]] <- FindVariableFeatures(dt.list[[i]], selection.method = "vst", nfeatures = 2000, verbose = FALSE)
}
...
#Get all markers, without logFC filter
all_markers_LSC_AML6 <- FindMarkers(integrated.obj1, ident.1 = "AML6_Dx_11", ident.2 = "AML6_Rx_10", only.pos = FALSE, logfc.threshold = 0)
all_markers_LSC_AML6$cluster <- unlist(lapply(all_markers_LSC_AML6$avg_log2FC,function(x)ifelse(x>0,"AML6_Dx_11","AML6_Rx_10")))
all_markers_LSC_AML6$avg_log2FC <- abs(all_markers_LSC_AML6$avg_log2FC)
all_markers_LSC_AML6$gene <- rownames(all_markers_LSC_AML6)
table(all_markers_LSC_AML6[which(all_markers_LSC_AML6$avg_log2FC>=0.25),]$cluster)
...
aux_df1$LSC_status <- "NonLSC 38+"
aux_df1$LSC_status[which(aux_df1$Sorting=="CD34")] <- "NonLSC 34+"
aux_df1$LSC_status[which(aux_df1$old_clustering=="AML6_Dx_11" | aux_df1$old_clustering=="AML6_Rx_10")] <- "LSC 34+"
aux_df1$LSC_status <- factor(aux_df1$LSC_status, levels=c("LSC 34+","NonLSC 34+","NonLSC 38+"))
aux_df1$LSC_status2 <- "Xx_LSC 34+"
aux_df1$LSC_status2[which(aux_df1$LSC_status=="LSC 34+" & aux_df1$Condition=="Dx")] <- "Dx_LSC 34+"
aux_df1$LSC_status2[which(aux_df1$LSC_status=="LSC 34+" & aux_df1$Condition=="Rx")] <- "Rx_LSC 34+"
...
LSC_status <- rep("NonLSC 38+",ncol(integrated.obj1))
LSC_status[which(integrated.obj1$Sample_id=="CD34")] <- "NonLSC 34+"
LSC_status[which(integrated.obj1$old_clustering=="AML6_Dx_11" | integrated.obj1$old_clustering=="AML6_Rx_10")] <- "LSC 34+"
LSC_status <- factor(LSC_status, levels=c("LSC 34+","NonLSC 34+","NonLSC 38+"))
names(LSC_status) <- colnames(integrated.obj1)
integrated.obj1 <- AddMetaData(object = integrated.obj1, metadata = LSC_status, col.name = "LSC_status")
...
## 3. AML11_Dx-AML11_Rx: Take the LSC clusters we derived independently

```{r part3, echo=FALSE, fig.height=8, fig.width=4, message=FALSE}
library(Seurat) 
library(ggplot2) 
library(ggExtra) 
library(dplyr)
...
#### 1. Load samples processed from AML11_Dx.Rmd and AML11_Rx.Rmd
AML11_Dx$orig.name <- "AML11_Dx"
AML11_Dx$old_clustering <- paste0("AML11_Dx_",Idents(AML11_Dx))
AML11_Dx$Cond <- "Dx"

AML11_Rx$orig.name <- "AML11_Rx"
AML11_Rx$old_clustering <- paste0("AML11_Rx_",Idents(AML11_Rx))
AML11_Rx$Cond <- "Rx"

#### 2.  Integrate the datasets
dt.list <- unlist(list(c(AML11_Dx,AML11_Rx)))
for (i in 1:length(dt.list)) {
    dt.list[[i]] <- NormalizeData(dt.list[[i]], verbose = FALSE)
    dt.list[[i]] <- FindVariableFeatures(dt.list[[i]], selection.method = "vst", nfeatures = 2000, verbose = FALSE)
}
...
#Get all markers, without logFC filter
all_markers_LSC_AML11 <- FindMarkers(integrated.obj1, ident.1 = "AML11_Dx_2", ident.2 = "AML11_Rx_2", only.pos = FALSE, logfc.threshold = 0)
all_markers_LSC_AML11$cluster <- unlist(lapply(all_markers_LSC_AML11$avg_log2FC,function(x)ifelse(x>0,"AML11_Dx_2","AML11_Rx_2")))
all_markers_LSC_AML11$avg_log2FC <- abs(all_markers_LSC_AML11$avg_log2FC)
all_markers_LSC_AML11$gene <- rownames(all_markers_LSC_AML11)
table(all_markers_LSC_AML11[which(all_markers_LSC_AML11$avg_log2FC>=0.25),]$cluster)

```

### 3.1 Check the differences between Dx/Rx on the LSC6 and Hypoxia

```{r part3.1, echo=FALSE, fig.height=5, fig.width=5, message=FALSE}
library(ggpubr)
...
aux_df1$LSC_status <- "NonLSC 38+"
aux_df1$LSC_status[which(aux_df1$Sorting=="CD34")] <- "NonLSC 34+"
aux_df1$LSC_status[which(aux_df1$old_clustering=="AML11_Dx_2" | aux_df1$old_clustering=="AML11_Rx_2")] <- "LSC 34+"
aux_df1$LSC_status <- factor(aux_df1$LSC_status, levels=c("LSC 34+","NonLSC 34+","NonLSC 38+"))
aux_df1$LSC_status2 <- "Xx_LSC 34+"
aux_df1$LSC_status2[which(aux_df1$LSC_status=="LSC 34+" & aux_df1$Condition=="Dx")] <- "Dx_LSC 34+"
aux_df1$LSC_status2[which(aux_df1$LSC_status=="LSC 34+" & aux_df1$Condition=="Rx")] <- "Rx_LSC 34+"
...
### 3.2 Test other signatures and the difference between Dx/Rx

```{r part3.2, echo=FALSE, fig.height=5, fig.width=6, message=FALSE}
library(ggpubr)
library(UCell)
library(xlsx)
...
LSC_status <- rep("NonLSC 38+",ncol(integrated.obj1))
LSC_status[which(integrated.obj1$Sample_id=="CD34")] <- "NonLSC 34+"
LSC_status[which(integrated.obj1$old_clustering=="AML11_Dx_2" | integrated.obj1$old_clustering=="AML11_Rx_2")] <- "LSC 34+"
LSC_status <- factor(LSC_status, levels=c("LSC 34+","NonLSC 34+","NonLSC 38+"))
names(LSC_status) <- colnames(integrated.obj1)
integrated.obj1 <- AddMetaData(object = integrated.obj1, metadata = LSC_status, col.name = "LSC_status")
...
## 4. Compare the markers from both AML4, AML6 and AML11

```{r part4, echo=FALSE, fig.height=8, fig.width=8, message=FALSE}
library(ggrepel)
library(VennDiagram)
library("biomaRt")
...
nrow(all_markers_LSC_AML4)
nrow(all_markers_LSC_AML6)
nrow(all_markers_LSC_AML11)
#Label which genes are up on Dx/Rx
all_markers_LSC_AML4$cluster2 <- "Dx"
all_markers_LSC_AML4$cluster2[which(all_markers_LSC_AML4$cluster!="AML4_Dx_9")] <- "Rx"
table(all_markers_LSC_AML4$cluster2)
all_markers_LSC_AML6$cluster2 <- "Dx"
all_markers_LSC_AML6$cluster2[which(all_markers_LSC_AML6$cluster!="AML6_Dx_11")] <- "Rx"
table(all_markers_LSC_AML6$cluster2)
all_markers_LSC_AML11$cluster2 <- "Dx"
all_markers_LSC_AML11$cluster2[which(all_markers_LSC_AML11$cluster!="AML11_Dx_2")] <- "Rx"
table(all_markers_LSC_AML11$cluster2)
...
x <- list(
  AML4_Dx_Rx = all_markers_LSC_AML4$gene, 
  AML6_Dx_Rx = all_markers_LSC_AML6$gene,
  AML11_Dx_Rx = all_markers_LSC_AML11$gene
  )

display_venn(
  x,
  category.names = c("\t\t\tAML4_Dx_Rx" , "AML6_Dx_Rx\t\t\t\n\n", "AML11_Dx_Rx"),
  fill = c("#56B4E9", "#E69F00", "#0A9F00"),
  cex = 2.,
  cat.cex = 2.5
  )


temp <- rnorm(10, mean=30, sd=5)
pressure <- rnorm(10)
dtime <- 1:10
plot_ly(x=temp, y=pressure, z=dtime, type="scatter3d", mode="markers", color=temp)

#Do a deregulogram
all_markers_LSC_AML4$log2FC <- all_markers_LSC_AML4$avg_log2FC
all_markers_LSC_AML4$log2FC[which(all_markers_LSC_AML4$cluster=="AML4_9")] <- -all_markers_LSC_AML4$log2FC[which(all_markers_LSC_AML4$cluster=="AML4_9")]
all_markers_LSC_AML6$log2FC <- all_markers_LSC_AML6$avg_log2FC
all_markers_LSC_AML6$log2FC[which(all_markers_LSC_AML6$cluster=="AML6_11")] <- -all_markers_LSC_AML6$log2FC[which(all_markers_LSC_AML6$cluster=="AML6_11")]
all_markers_LSC_AML11$log2FC <- all_markers_LSC_AML11$avg_log2FC
all_markers_LSC_AML11$log2FC[which(all_markers_LSC_AML11$cluster=="AML11_2")] <- -all_markers_LSC_AML11$log2FC[which(all_markers_LSC_AML11$cluster=="AML11_2")]

merge1 <- merge(all_markers_LSC_AML4,all_markers_LSC_AML6,by.x="gene",by.y="gene")
merge2 <- as.data.frame(merge(merge1,all_markers_LSC_AML11,by.x="gene",by.y="gene"))
rownames(merge2) <- merge2$gene
colnames(merge2)[c(9,17,25)] <- c("log2FC.AML4","log2FC.AML6","log2FC.AML11")
fig <- plot_ly(x=merge2$log2FC.AML4, y=merge2$log2FC.AML6, z=merge2$log2FC.AML11, text = merge2$gene, type="scatter3d", mode="markers", color=merge2$log2FC.AML6, marker = list(size = abs(merge2$log2FC.AML6)))
fig <- fig %>% layout(title = 'rMLL Dx/Rx DE genes comparison',
         xaxis = list(showgrid = FALSE),
         yaxis = list(showgrid = FALSE))
fig

merge2$color <- rep("#FFFFFF",nrow(merge2))
same_direction.AML4_AML6 <- merge2 %>% filter((log2FC.AML4 > 0.5 & log2FC.AML6 > 0.5) | (log2FC.AML4 < -0.5 & log2FC.AML6 < -0.5))
same_direction.AML4_AML11 <- merge2 %>% filter((log2FC.AML4 > 0.5 & log2FC.AML11 > 0.5) | (log2FC.AML4 < -0.5 & log2FC.AML11 < -0.5))
same_direction.AML6_AML11 <- merge2 %>% filter((log2FC.AML6 > 0.5 & log2FC.AML11 > 0.5) | (log2FC.AML6 < -0.5 & log2FC.AML11 < -0.5))
merge2$color[which(merge2$gene%in%same_direction.AML4_AML6$gene)] <- "#f00000"
merge2$color[which(merge2$gene%in%same_direction.AML4_AML11$gene)] <- "#b54b00"
merge2$color[which(merge2$gene%in%same_direction.AML6_AML11$gene)] <- "#ba8c00"
merge2$avg_log2FC.all <- (merge2$avg_log2FC.x+merge2$avg_log2FC.y+merge2$avg_log2FC)
fig <- plot_ly(merge2, x=~log2FC.AML4, y=~log2FC.AML6, z=~log2FC.AML11, text = ~gene, type="scatter3d", color=~avg_log2FC.all, marker = list(size = ~avg_log2FC.all))
fig <- fig %>% layout(title = 'rMLL Dx/Rx DE genes comparison',
         xaxis = list(showgrid = FALSE),
         yaxis = list(showgrid = FALSE),
...
#Sort the list by the avg of all log2FC
merge2$log2FC.allsum <- unlist(apply(merge2,1,function(x)as.numeric(x[9])+as.numeric(x[17])+as.numeric(x[25])))
merge2_sorted <-merge2[order(-merge2$log2FC.allsum),]
merge2_sorted$position_x <- seq(0,nrow(merge2_sorted)-1)
#Color the dots: red: all genes have log2FC negative. blue: all genes have log2FC positive
merge2_sorted$color <- "red"
merge2_sorted$color[which((merge2_sorted$log2FC.AML4>0 & merge2_sorted$log2FC.AML6>0 & merge2_sorted$log2FC.AML11>0) | (merge2_sorted$log2FC.AML4<0 & merge2_sorted$log2FC.AML6<0 & merge2_sorted$log2FC.AML11<0))] <- "blue"
#If any gene is under abs(sumlogFC) put it in black
merge2_sorted$color[which(abs(merge2_sorted$log2FC.allsum)<0.25)] <- "black"

#Label the top20 genes up and down, if they're blue dots
genes_text <- c(head(merge2_sorted[which(merge2_sorted$color=="blue"),]$gene,n=20L),head(rev(merge2_sorted[which(merge2_sorted$color=="blue"),]$gene),n=20L))
merge2_sorted$gene[which(!merge2_sorted$gene%in%genes_text)] <- ""

#Plot all the genes 
deregulogram_plot <- ggplot(merge2_sorted,aes(x=position_x,y=log2FC.allsum,label=gene)) +
  geom_point(aes(colour=factor(color))) + 
  scale_colour_manual(values = c("black","blue", "red")) +
  theme_bw() +
  geom_hline(yintercept=0.25,linetype="dashed") +
  geom_hline(yintercept=-0.25,linetype="dashed") +
  ylab("sum(log2FC(Rx/Dx))") + xlab("") +
  geom_text_repel(min.segment.length = 0, seed = 42, box.padding = 0.5, max.overlaps = 50) + 
  theme(axis.text.x = element_blank(),axis.ticks.x = element_blank(),legend.position = "none")
plot(deregulogram_plot)

#How many genes are consistently up or down regulated
merge3 <-merge2_sorted[which(merge2_sorted$color=="blue"),]
merge3$up_Rx <- "up"
merge3$up_Rx[which(merge3$log2FC.AML4<0 & merge3$log2FC.AML6<0 & merge3$log2FC.AML11<0)] <- "down"
merge3$gene <- rownames(merge3)
table(merge3$up_Rx)

#Take only the genes in blue, the ones in agreement between pairs
#Enrich the genename of these genes
anno <- AnnotationDbi::select(org.Hs.eg.db, 
                              keys=merge3$gene, 
                              columns=c("SYMBOL", "GENENAME"),
                              keytype="SYMBOL")
merge3 <- merge(merge3,anno,by.x="gene",by.y="SYMBOL")
