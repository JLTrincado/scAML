...
# CD34+
CD34.data <- Read10X(data.dir = "~/AML4_Dx_CD34pos/filtered_feature_bc_matrix/")
...
p2 + geom_vline(xintercept=3.4) + geom_hline(yintercept=300) + geom_hline(yintercept=7000)

# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD34 <- subset(x = CD34, subset = nFeature_RNA > 300 & nFeature_RNA < 7000  & percent.mito < 0.2 & log10_nCount_RNA > 3.4)
CD34

# CD38+
CD38.data <- Read10X(data.dir = "~/AML4_Dx_CD38pos/filtered_feature_bc_matrix/")
...
p2 + geom_vline(xintercept=lower_bound) + geom_vline(xintercept=upper_bound) + geom_hline(yintercept=300) + geom_hline(yintercept=7000)

# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD38 <- subset(x = CD38, subset = nFeature_RNA > 300 & percent.mito < 0.2 & log10_nCount_RNA > lower_bound)
CD38

#### 2. Combine the datasets #### 
combined.obj.AML <- merge(x = CD34, y = CD38, add.cell.ids = c("CD34","CD38") , project = "AML4_Dx")
...
#PCA
DimPlot(combined.obj.AML, reduction = "pca")

#UMAP
combined.obj.AML <- RunUMAP(combined.obj.AML, dims = 1:n_PC)
# Plot
DimPlot(combined.obj.AML, reduction = "umap",  pt.size = 1.2)  
...
#Filter the AML4_Dx cells
Velten_proj_f <- Velten_proj[which(Velten_proj$patient=="AML4"),]
#Format the bc
