...
AML1$orig.name <- "AML1"
AML1$old_clustering <- paste0("AML1_",Idents(AML1))
...
```{r part7, fig.height=3, fig.width=5, message=FALSE}
library(ReactomePA)
library(forcats)
library(ggplot2)
library(ggstance)
...
markers <- read.csv(file="~/markers.rMLL.LSCvsNonLSC34pos.csv")
...
##1.2. LSC34+ vs 38+
markers <- read.csv(file="~/markers.rMLL.LSCvsNonLSC34vs38.csv")
markers_f <- markers[which(markers$cluster=="LSC 34+" | markers$cluster=="38+"),]
...
markers_f$avg_log2FC_aux[which(markers_f$cluster=="38+")] <- -markers_f$avg_log2FC_aux[which(markers_f$cluster=="38+")]
...
##1.3. NonLSC34+ vs 38+
markers <- read.csv(file="/media/trincadojl/data/Projects/MENENDEZPAB_03/misc/markers.rMLL.LSCvsNonLSC34vs38.csv")
markers_f <- markers[which(markers$cluster=="NonLSC 34+" | markers$cluster=="38+"),]
