...
# CD34+
CD34.data <- Read10X(data.dir = "~/AML8_Dx_CD34pos/filtered_feature_bc_matrix/")
...
CD34 <- subset(x = CD34, subset = nFeature_RNA > 200 & percent.mito < 0.2 & log10_nCount_RNA > 3.5)
CD34

# CD38+
CD38.data <- Read10X(data.dir = "~/AML8_Dx_CD38pos/filtered_feature_bc_matrix/")
...
s.genes <- cc.genes[1:43]
g2m.genes <- cc.genes[44:97]
CD38 <- CellCycleScoring(object = CD38, s.genes, g2m.genes,set.ident = FALSE, nbin = 20)
...
# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD38 <- subset(x = CD38, subset = nFeature_RNA > 200 & percent.mito < 0.2 & log10_nCount_RNA > 3.25)
CD38

#### 2. Combine the datasets ####
combined.obj.AML <- merge(x = CD34, y = CD38, add.cell.ids = c("CD34","CD38") , project = "AML8_Dx")
...
#Filter the AML8_Dx cells (these are AML9 in Velten's table)
Velten_proj_f <- Velten_proj[which(Velten_proj$patient=="AML9"),]
#Format the bc
