...
# CD34+
CD34.data <- Read10X(data.dir = "~/AML3_11387_CD34pos/filtered_feature_bc_matrix/")
...
CD34 <- CellCycleScoring(object = CD34, s.genes, g2m.genes,set.ident = FALSE, nbin = 20)
...
p1 + geom_vline(xintercept=3.4) + geom_hline(yintercept=0.2)
p2 + geom_vline(xintercept=3.4) + geom_hline(yintercept=200) + geom_hline(yintercept=5500)

# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD34 <- subset(x = CD34, subset = nFeature_RNA > 200 & nFeature_RNA < 5500  & percent.mito < 0.2 & log10_nCount_RNA > 3.4)
CD34

# CD38+
CD38.data <- Read10X(data.dir = "~/AML3_11387_CD38pos/filtered_feature_bc_matrix/")
...
CD38$Sample_id <- "CD38"
# Read in a list of cell cycle markers, from Tirosh et al, 2015
cc.genes <- readLines(con = "/home/trincadojl/Projects/Bigas/Initial/tables/regev_lab_cell_cycle_genes.txt")
# We can segregate this list into markers of G2/M phase and markers of S phase
s.genes <- cc.genes[1:43]
g2m.genes <- cc.genes[44:97]
CD38 <- CellCycleScoring(object = CD38, s.genes, g2m.genes,set.ident = FALSE)
...
combined.obj.AML <- merge(x = CD34, y = CD38, add.cell.ids = c("CD34","CD38") , project = "AML3")
...
combined.obj.AML <- FindClusters(combined.obj.AML, resolution = 0.5)
head(Idents(combined.obj.AML), 5)

#UMAP
combined.obj.AML <- RunUMAP(combined.obj.AML, dims = 1:n_PC)
# Plot
n_cluster = length(unique(Idents(combined.obj.AML)))
getPalette = colorRampPalette(brewer.pal(n_cluster, "Set3"))
colors = getPalette(n_cluster)
DimPlot(combined.obj.AML, reduction = "umap",  pt.size = 1.2) + scale_color_manual(values = colors) + NoAxes()
...
LSC6_UMAP <- FeaturePlot(combined.obj.AML,features = "Elsayed_LSC_score", pt.size = 1, order = T) + NoAxes()

df1 <- data.frame(clusters=Idents(combined.obj.AML),Elsayed_LSC_score=combined.obj.AML$Elsayed_LSC_score)
ggplot(df1,aes(x=clusters,y=Elsayed_LSC_score, fill=clusters)) +
  geom_boxplot(outlier.shape = NA) +
  theme_classic() +
  NoLegend() +
  scale_y_continuous(limits = quantile(df1$Elsayed_LSC_score, c(0.1, 0.9)))
...
aux_df <- data.frame(Condtion=combined.obj.AML$Sample_id,Predicted_cell_type=combined.obj.AML$prediction,Clusters=Idents(combined.obj.AML),Elsayed_LSC_score=combined.obj.AML$Elsayed_LSC_score)
...
ggplot(aux_df, aes(Clusters,fill=Predicted_cell_type)) +
  geom_bar() + 
  scale_fill_manual(values = colors)
#Sort the barplot according to LSC6 score
ggplot(aux_df, aes(x=fct_reorder(Clusters, Elsayed_LSC_score, .fun = median, .desc = TRUE),fill=Predicted_cell_type)) +
  geom_bar() + 
  scale_fill_manual(values = colors) +
  theme_bw()
...
Velten_proj <- read.xlsx(xlsxFile="/media/trincadojl/data/Projects/MENENDEZPAB_03/table/Velten_projections.xlsx")
...
table(Velten_proj$celltype,Velten_proj$celltype2)
...
#Filter the AML3 cells
Velten_proj_f <- Velten_proj[which(Velten_proj$patient=="AML3"),]
#Format the bc
...
Velten_proj_f$cellBC2 <- apply(Velten_proj_f,1,function(x)paste0(x[7],"_",strsplit(x[10],"-")[[1]][1]))
...
df <- data.frame(cellBC=colnames(combined.obj.AML),suffix=unlist(lapply(colnames(combined.obj.AML),function(x)strsplit(x,"_")[[1]][3])),Sorting=combined.obj.AML$Sample_id,celltype=combined.obj.AML$celltype,Clusters=Idents(combined.obj.AML),Elsayed_LSC_score=combined.obj.AML$Elsayed_LSC_score)
