...
# CD34+
CD34.data <- Read10X(data.dir = "~/AML5_CD34pos/filtered_feature_bc_matrix/")
...
p2 + geom_vline(xintercept=lower_bound) + geom_hline(yintercept=400) + geom_hline(yintercept=7000)

# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD34 <- subset(x = CD34, subset = nFeature_RNA > 400 & nFeature_RNA < 7000  & percent.mito < 0.2 & log10_nCount_RNA > lower_bound)
CD34

# CD38+
CD38.data <- Read10X(data.dir = "~/AML5_CD38pos/filtered_feature_bc_matrix/")
...
p2 + geom_vline(xintercept=lower_bound) + geom_vline(xintercept=4.5) + geom_hline(yintercept=400) + geom_hline(yintercept=5500)

# We filter out cells that have unique gene counts less than 200 and cells too expressed (could be doublets)
CD38 <- subset(x = CD38, subset = nFeature_RNA > 400 & nFeature_RNA < 5500 & percent.mito < 0.2 & log10_nCount_RNA > lower_bound & log10_nCount_RNA < 4.5)
CD38

#### 2. Combine the datasets #### 
combined.obj.AML <- merge(x = CD34, y = CD38, add.cell.ids = c("CD34","CD38") , project = "AML5")
...
```{r part3, eval=FALSE, fig.height=10, fig.width=12, message=FALSE, include=FALSE}
library(Seurat) 
library(ggplot2) 
library(dplyr)
library(data.table)
...
#Filter the AML5 cells
Velten_proj_f <- Velten_proj[which(Velten_proj$patient=="AML5"),]
#Format the bc
