...
## 1. AML7_Dx-AML7_Rx: Take the LSC clusters we derived independently
...
#### 1. Load samples processed from AML7_Dx.Rmd and AML7_Rx.Rmd
AML7_Dx$orig.name <- "AML7_Dx"
AML7_Dx$old_clustering <- paste0("AML7_Dx_",Idents(AML7_Dx))
AML7_Dx$Cond <- "Dx"

AML7_Rx$orig.name <- "AML7_Rx"
AML7_Rx$old_clustering <- paste0("AML7_Rx_",Idents(AML7_Rx))
AML7_Rx$Cond <- "Rx"

#### 2.  Integrate the datasets
dt.list <- unlist(list(c(AML7_Dx,AML7_Rx)))
...
all_markers_LSC_AML7 <- FindMarkers(integrated.obj1, ident.1 = "AML7_Dx_0", ident.2 = "AML7_Rx_0", only.pos = FALSE, logfc.threshold = 0)
all_markers_LSC_AML7$cluster <- unlist(lapply(all_markers_LSC_AML7$avg_log2FC,function(x)ifelse(x>0,"AML7_Dx_0","AML7_Rx_0")))
all_markers_LSC_AML7$avg_log2FC <- abs(all_markers_LSC_AML7$avg_log2FC)
all_markers_LSC_AML7$gene <- rownames(all_markers_LSC_AML7)
table(all_markers_LSC_AML7[which(all_markers_LSC_AML7$avg_log2FC>=0.25),]$cluster)
...
aux_df1$LSC_status[which(aux_df1$old_clustering=="AML7_Dx_0" | aux_df1$old_clustering=="AML7_Rx_0")] <- "LSC 34+"
...
LSC_status[which(integrated.obj1$old_clustering=="AML7_Dx_0" | integrated.obj1$old_clustering=="AML7_Rx_0")] <- "LSC 34+"
...
## 2. AML10_Dx-AML10_Rx: Take the LSC clusters we derived independently
...
#### 1. Load samples processed from AML10_Dx.Rmd and AML10_Rx.Rmd
AML10_Dx$orig.name <- "AML10_Dx"
AML10_Dx$old_clustering <- paste0("AML10_Dx_",Idents(AML10_Dx))
AML10_Dx$Cond <- "Dx"

AML10_Rx$orig.name <- "AML10_Rx"
AML10_Rx$old_clustering <- paste0("AML10_Rx_",Idents(AML10_Rx))
AML10_Rx$Cond <- "Rx"

#### 2.  Integrate the datasets
dt.list <- unlist(list(c(AML10_Dx,AML10_Rx)))
for (i in 1:length(dt.list)) {
    dt.list[[i]] <- NormalizeData(dt.list[[i]], verbose = FALSE)
    dt.list[[i]] <- FindVariableFeatures(dt.list[[i]], selection.method = "vst", nfeatures = 2000, verbose = FALSE)
}
...
#Get all markers, without logFC filter
all_markers_LSC_AML10 <- FindMarkers(integrated.obj1, ident.1 = "AML10_Dx_5", ident.2 = "AML10_Rx_4", only.pos = FALSE, logfc.threshold = 0)
all_markers_LSC_AML10$cluster <- unlist(lapply(all_markers_LSC_AML10$avg_log2FC,function(x)ifelse(x>0,"AML10_Dx_5","AML10_Rx_4")))
all_markers_LSC_AML10$avg_log2FC <- abs(all_markers_LSC_AML10$avg_log2FC)
all_markers_LSC_AML10$gene <- rownames(all_markers_LSC_AML10)
table(all_markers_LSC_AML10[which(all_markers_LSC_AML10$avg_log2FC>=0.25),]$cluster)
...
aux_df1$LSC_status <- "NonLSC 38+"
aux_df1$LSC_status[which(aux_df1$Sorting=="CD34")] <- "NonLSC 34+"
aux_df1$LSC_status[which(aux_df1$old_clustering=="AML10_Dx_5" | aux_df1$old_clustering=="AML10_Rx_4")] <- "LSC 34+"
aux_df1$LSC_status <- factor(aux_df1$LSC_status, levels=c("LSC 34+","NonLSC 34+","NonLSC 38+"))
aux_df1$LSC_status2 <- "Xx_LSC 34+"
aux_df1$LSC_status2[which(aux_df1$LSC_status=="LSC 34+" & aux_df1$Condition=="Dx")] <- "Dx_LSC 34+"
aux_df1$LSC_status2[which(aux_df1$LSC_status=="LSC 34+" & aux_df1$Condition=="Rx")] <- "Rx_LSC 34+"
...
LSC_status <- rep("NonLSC 38+",ncol(integrated.obj1))
LSC_status[which(integrated.obj1$Sample_id=="CD34")] <- "NonLSC 34+"
LSC_status[which(integrated.obj1$old_clustering=="AML10_Dx_5" | integrated.obj1$old_clustering=="AML10_Rx_4")] <- "LSC 34+"
LSC_status <- factor(LSC_status, levels=c("LSC 34+","NonLSC 34+","NonLSC 38+"))
names(LSC_status) <- colnames(integrated.obj1)
integrated.obj1 <- AddMetaData(object = integrated.obj1, metadata = LSC_status, col.name = "LSC_status")
...
nrow(all_markers_LSC_AML7)
nrow(all_markers_LSC_AML10)
#Label which genes are up on Dx/Rx
all_markers_LSC_AML7$cluster2 <- "Dx"
all_markers_LSC_AML7$cluster2[which(all_markers_LSC_AML7$cluster!="AML7_Dx_0")] <- "Rx"
table(all_markers_LSC_AML7$cluster2)
all_markers_LSC_AML10$cluster2 <- "Dx"
all_markers_LSC_AML10$cluster2[which(all_markers_LSC_AML10$cluster!="AML10_Dx_5")] <- "Rx"
table(all_markers_LSC_AML10$cluster2)
...
x <- list(
  AML7_Dx_Rx = all_markers_LSC_AML7$gene, 
  AML10_Dx_Rx = all_markers_LSC_AML10$gene
  )

display_venn(
  x,
  category.names = c("\t\tAML7_Dx_Rx" , "AML10_Dx_Rx\t\t\t\n\n"),
...
#Do a deregulogram
all_markers_LSC_AML7$log2FC <- all_markers_LSC_AML7$avg_log2FC
all_markers_LSC_AML7$log2FC[which(all_markers_LSC_AML7$cluster=="AML7_Dx_0")] <- -all_markers_LSC_AML7$log2FC[which(all_markers_LSC_AML7$cluster=="AML7_Dx_0")]
all_markers_LSC_AML10$log2FC <- all_markers_LSC_AML10$avg_log2FC
all_markers_LSC_AML10$log2FC[which(all_markers_LSC_AML10$cluster=="AML10_Dx_5")] <- -all_markers_LSC_AML10$log2FC[which(all_markers_LSC_AML10$cluster=="AML10_Dx_5")]
merge <- merge(all_markers_LSC_AML7,all_markers_LSC_AML10,by.x="gene",by.y="gene")
...
deregulogram_plot <- ggplot(merge,aes(label=gene,x=log2FC.x,y=log2FC.y)) +
  ...
  geom_vline(xintercept=-0.25,linetype="dashed") +
  xlab("log2FC(AML7_Rx - AML7_Dx)") +
  ylab("log2FC(AML10_Rx - AML10_Dx)") + 
